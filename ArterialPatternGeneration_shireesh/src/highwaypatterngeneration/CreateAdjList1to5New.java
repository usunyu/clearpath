...
public class CreateAdjList1to5New {
	...
	static String FILE_LINK = "H:\\Jiayunge\\Edges_G_12345_final_extraStnames.csv";
	...
	//static HashMap<String, String>[] speedsinfo = new HashMap<String, String>[7];
	...
	public static void readPreprocessedFiles(int index, String day){
		try{
			...
			FileInputStream fstream = new FileInputStream("H:\\Jiayunge\\AverageEdgeSpeed_"+day+".txt");
		    ...
		    fstream = new FileInputStream("H:\\Jiayunge\\HighwayEdgeSpeed_"+day+".txt");
		...
		for(int i = 0; i <SpeedNodes.get(index).size(); i++){
		  ...
		  //Check if the end node is V on anything !=1000.0
		    ...
		    if(count == 60)
		    {
		    	...
		    	outNodes[stNodeId].append(ts.getEndNode()+"(F):"+(int)(travelTime[0]*60.0*1000.0)+";");
		    	continue;
		    }
		    ...
		    for(int k=0;k<60;k++)
		     {
		    	 ...
		    	 else
		    	 {
		    		 if(k!=59)
		    			 ...
		    			 outNodes[stNodeId].append((int)(travelTime[k]*60.0*1000.0)+",");
			    		 else
			    		 //out.write((int)(travelTime[k]*60.0*1000.0)+";");
			    		 outNodes[stNodeId].append((int)(travelTime[k]*60.0*1000.0)+";");	 
		    	 }
	...
	private static double[] getTravelTimeFromFile(int index, String day,LinkInfo link) {
