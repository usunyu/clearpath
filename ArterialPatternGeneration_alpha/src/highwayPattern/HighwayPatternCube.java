...
public class HighwayPatternCube {
	...
	public static void main(String[] args) {
		...
		matchLinkSensor();
		readAverageFile(6, 0);
//		writeAverageFile(6, 0);
		generatePatternKML(102, 6, 0);
	}
	
	private static void generatePatternKML(int timeIndex, int month, int day) {
		System.out.println("generate pattern...");
		int error = 0;
		try {
			FileWriter fstream = new FileWriter(root + "/" + months[month] + "_" + days[day] + "_" + highwayPatternKML);
	...
	private static void writeAverageFile(int month, int day) {
			...
			for (int j = 0; j < refAdjList.size(); j++) {
				...
				if (nodeToLink.containsKey(nodesStr)) {
					...
					if (nearLink.getSensorList().size() == 0) {
						...
						double minDis = DistanceCalculator
								.CalculationByDistance(nearNode,
										nearestSensor.getNode());
						for (int k = 1; k < linkSensorList.size(); k++) {
							SensorInfo otherSensor = linkSensorList.get(k);
							double dis = DistanceCalculator
									.CalculationByDistance(nearNode,
											otherSensor.getNode());
							if (dis < minDis) {
